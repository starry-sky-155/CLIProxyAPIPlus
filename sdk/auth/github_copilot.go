...
func (a GitHubCopilotAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	// Verify the token is valid by checking user info
	fmt.Println("Verifying Copilot access...")
	valid, _, err := authSvc.ValidateToken(ctx, authBundle.TokenData.AccessToken)
	if err != nil || !valid {
		if err == nil {
			err = fmt.Errorf("token validation returned false")
		}
